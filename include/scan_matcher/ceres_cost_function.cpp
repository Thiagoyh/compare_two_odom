                                        ...
                                        double *residuals, double **jacobians) const
{
    ...
    if(jacobians != NULL)
    {
        if(jacobians[0] != NULL)
        {
            ...
            J_se3.block<1,6>(0,0) = - nu.transpose() / nu.norm() *
                                     skew_de * dp_by_se3/de_norm;
...
SurfNormAnalyticCostFunction::SurfNormAnalyticCostFunction(
                     Eigen::Vector3d curr_point_, Eigen::Vector3d plane_unit_norm_,
...
ceres::CostFunction *CreateEdgeCostFunction(const Eigen::Vector3d curr_point,
    ...
    return new EdgeAnalyticCostFunction(curr_point, point_a, point_b);
}
ceres::CostFunction *CreateSurfCostFunction(const Eigen::Vector3d curr_point,
    ...
    return new SurfNormAnalyticCostFunction(curr_point, norm, negative_OA_dot_norm);
}
